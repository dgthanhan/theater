var DataTable = function () {
    ...
    ActionColumn.prototype.getCellContentHtml = function (data, row, col) {
        ...
        for (var i = 0; i < this.actions.length; i ++) {
            ...
            html += "<span action-id=\"" + action.id + "\" class=\"" + action.type + " Action"  + action.id +  " " + (action.isApplicable && action.isApplicable(data) ? "" : "disabled")  + "\" title=\"" + Dom.htmlEncode(action.title) + "\"><i ui-icon=\"" + action.type +"\"></i></span>"
        }
        return html;
    };
